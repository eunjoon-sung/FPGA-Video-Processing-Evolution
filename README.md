...
* **Hypothesis (Architectural Deduction):** Without clear ILA triggers across the asynchronous boundary, an architectural hypothesis was formulated:
  1. **Clock Frequency Drift (Plesiochronous Mismatch):** The precise 5-way split observed during the static color bar test strongly implied a microscopic difference between the camera's external `PCLK` and the FPGA's internal system clock (`25MHz`). While an Async FIFO safely crosses data between domains, it cannot absorb continuous frequency drift in a real-time streaming architecture without a full-frame buffer. This slight mismatch caused the horizontal sync to continuously drift and wrap around 5 times per frame.
  2. **Data-Control Decoupling (Phase Skew):** Suspected a critical architectural flaw where the Data path and Control path were structurally decoupled. Pixel data suffered variable latency as it queued through the Async FIFO, whereas control signals (`VSYNC`/`HREF`) bypassed the FIFO and propagated instantly. This timing mismatch caused severe phase skew: the SRAM address reset triggered prematurely based on the instant `VSYNC`, while the corresponding tail-end pixels were still trapped inside the FIFO, utterly destroying the frame structure.
