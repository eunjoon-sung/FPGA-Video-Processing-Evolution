...
* **Hypothesis (Architectural Deduction):** Without clear ILA triggers across the asynchronous boundary, an architectural hypothesis was formulated:
  1. **Clock Frequency Drift & Severe Tearing:** The precise 5-way split observed during the static color bar test strongly implied a microscopic frequency difference between the camera's external `PCLK` and the FPGA's internal system clock (`25MHz`). An Async FIFO is strictly a transitional tunnel for Clock Domain Crossing (CDC), not a macroscopic reservoir. Because the V1 architecture lacked a Full-Frame Buffer (Double/Triple buffering) to isolate the asynchronous read/write domains, this plesiochronous mismatch caused the camera's write pointer to continuously drift and overtake the display's read pointer. This structural lack of memory decoupling manifested as severe multi-line tearing (the 5-way split).
