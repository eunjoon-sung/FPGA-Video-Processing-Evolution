...
* **Root Cause (State Retention Overlap):** The ILA waveform (Figure right) revealed a critical data duplication issue (`0xFFAA_xFFAA` -> `[Pixel A][Pixel A][Pixel B][Pixel B]`). Reviewing the RTL code confirmed this was a sequential logic state retention issue, not an AXI bug. In the `camera_capture` module's 8-to-16-bit word assembly sequential block, there was no default assignment to pull `pixel_valid` low during the first byte capture. Consequently, the flip-flop simply retained its high state from the previous second-byte capture. This caused the `valid` strobe to erroneously remain asserted for *two* consecutive clock cycles per pixel, forcing the AXI writer to sample and write the same pixel twice and advancing the memory pointer at 2x speed.
* **Resolution:** Implemented a default sequential assignment (`pixel_valid <= 0;`) at the top of the active data parsing block. This ensures the register defaults to low every clock cycle unless explicitly overridden by a valid 16-bit word completion. This RTL refactoring guaranteed a strict single-cycle strobe, instantly restoring the memory mapping to a perfect 1:1 pixel-to-address ratio.
---
