...
* **Hypothesis (Architectural Deduction):** Without clear ILA triggers across the asynchronous boundary, an architectural hypothesis was formulated:
  1. **Clock Frequency Drift & FIFO Overflow/Underrun (The 5-way Split):** The precise 5-way split observed during the static color bar test strongly implied a microscopic frequency difference between the camera's external `PCLK` and the internal read clock (`25MHz`) at the Async FIFO boundary. Because an Async FIFO has a minimal depth (e.g., 1024 words), it can safely cross clock domains but cannot absorb macroscopic, continuous frequency drift over an entire frame (300,000+ pixels). This slight clock mismatch caused the FIFO to progressively accumulate drift until it either completely starved (underrun) or overflowed, exactly 5 times per frame. Each buffer failure broke the linear pixel stream, manifesting directly as a severed boundary (the 5-way split) on the monitor.

  2. **Data-Control Decoupling & Phase Skew:** Suspected a critical architectural flaw where the Data path and Control path were structurally decoupled across this CDC boundary. Pixel data inherently suffered pipeline latency as it queued through the Async FIFO, whereas control signals (`VSYNC`/`HREF`) bypassed the FIFO and were instantly synchronized to the 25MHz domain via 2-stage Flip-Flops. This timing mismatch caused severe phase skew: the internal logic prematurely reset its coordinate counters based on the instant `VSYNC` pulse, while the corresponding tail-end pixels from the previous frame were still trapped inside the FIFO. This misalignment utterly destroyed the frame's spatial structure, resulting in the noisy, unrecognizable output.
